...
contract SupplyChain {
    ...
    function addDistOrReta(
        address _address,
        uint256 _type,
        string memory _poistion
    ...
    function verifythedata(uint256 _batchcode, bool _vote) public {
        ...
        require(
            distributors[msg.sender].verified == true ||
                retailer[msg.sender].verified == true,
            "The Voter is not verified"
        );
        if (distributors[msg.sender].verified == true) {
            verifydata[_batchcode][
                batch[_batchcode].verifycount++
            ] = VerifyData(
                distributors[msg.sender].position,
                msg.sender,
                block.timestamp,
                _vote
            );
    ...
    function viewverifiedData(uint256 _batchcode, uint256 _verifierID)
        public
        view
        returns (
            string memory,
    ...
    {
        return (
            verifydata[_batchcode][_verifierID].position,
            verifydata[_batchcode][_verifierID].verifier,
            verifydata[_batchcode][_verifierID].date,
            verifydata[_batchcode][_verifierID].vote
        );
    }

    function getMedicineData(uint256 _batchcode)
        public
        view
        returns (
            string memory,
    ...
    {
        return (
            batch[_batchcode].name,
            batch[_batchcode].date,
            batch[_batchcode].IPFSHash,
            batch[_batchcode].verifycount
        );
    }

    function getMedicineDoc(uint256 _batchcode)
        public
        view
